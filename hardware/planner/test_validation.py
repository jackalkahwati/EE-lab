"""Phase 14 regression: real validation reports from synthetic policy inputs.

Both branches of the legacy fixed/blocked policy are exercised. Mock evidence
must remain simulated. Historical cal-attempt build state remains a named failing
dependency until it can be regenerated without seeding the expected verdict.
...
for fixed_policy in (False, True):
    ...
    RD = policy_artifacts(fixed=fixed_policy, validation=True)
    ...
    # In-memory policy selector only, not a manufactured fine-grid report.
    ...
    # 24/25 is a historical linkage assertion, evaluated separately below.
...
# 24/25 Phase 13 unchanged. This original assertion concerns historical router
# output, not an input we may author. No expected build_recommendation is seeded.
...
historical_dependency = bool(
    _fg and cal_attempt and "build_recommendation" in cal_attempt.get("fine_pitch_escape", {}))
check("24/25 cal board build state matches reality (fixed -> review, else do_not_build)",
      historical_dependency and
      cal_attempt["fine_pitch_escape"]["build_recommendation"] ==
      ("ready_to_build_with_review" if FIXED else "do_not_build"),
      "UNRESOLVED dependency: gen_finegrid cal-board-attempt build_recommendation; "
      "requires genuine board/DRC/run outputs, not a seeded policy verdict")

ib = art("fl1-internal-board-adapter-spec")
vr = art("fl1-validation-readiness-dashboard")
demos = art("demo-validation-runs")
cal_ib = next((b for b in ib["boards"] if b["board"] == "calibration_reference"), None) if ib else None
cal_vr = next((b for b in vr["boards"] if b["board"] == "calibration_reference"), None) if vr else None
byboard = {r["board_id"]: r for r in demos["runs"]} if demos else {}
check("historical finegrid/adapter/readiness/demo artifacts reconstructed by real producers",
      _fg is not None and cal_ib is not None and cal_vr is not None
      and "calibration_reference" in byboard,
      "UNRESOLVED dependency: genuine calibration reconstruction plus validation producer outputs")
check("8 cal board internal adapter honest (never physically available pre-fab)",
      cal_ib is not None and (
          not cal_ib["physically_available"] and
          (cal_ib["adapter_availability"] == "future_internal_board" if FIXED
           else cal_ib["adapter_availability"] in ("mock_only", "unsupported"))))
check("13 cal board validation readiness honest (mock-only unless truly fixed)",
      cal_vr is not None and (
          ((not cal_vr["physical_validation_blocked"]
            and cal_vr["internal_board_future_adapter"]) if FIXED
           else (cal_vr["validation_readiness_status"] == "validation_ready_with_mock"
                 and cal_vr["physical_validation_blocked"]
                 and not cal_vr["internal_board_future_adapter"]))))
check("17 cal mock run honest (never a physical pass; blocked unless fixed)",
      "calibration_reference" in byboard and (
          ("physical_blocked" in byboard["calibration_reference"]["physical_validation"])
          if not FIXED else
          ("not_attempted" in byboard["calibration_reference"]["physical_validation"]
           or "physical_blocked" in byboard["calibration_reference"]["physical_validation"])))
...
print("%d/%d validation checks pass (two synthetic policy scenarios; history dependency retained)" % (npass, len(checks)))
