...
def main():
    ...
    print()
    print("  Key findings (v4: k-paths + 5% tolerance + precision scoring + interior TPs + AC sweep):")
    print("  • OPEN/MISSING:   21-59% top-1. Voltage collapse is reliable; vote dilution")
    print("    from k-paths is the binding limit — parallel paths split suspicion.")
    print("    Cal board (15 TPs) reaches 59%; power board (9 TPs) stays at ~22%.")
    print("  • SHORT:          58-62% top-1. Best fault type; AC impedance sweep detects")
    print("    shorts invisible to DC. k-paths boosts power board from 51% → 62%.")
    print("  • WRONG_VALUE:    32-49% top-1. Visible in ratio measurements; pinning")
    print("    is hard when multiple components share a voltage divider path.")
    print("  • DEGRADED:        8-16% top-1. Near-random — fundamental limit of")
    print("    magnitude-only analysis. Needs phase/time-domain signatures.")
    print("  • BRIDGING:       60-78% — strongly correlated with interior TP density.")
    print("    Cal board (15 TPs) reaches 78%; power board (9 TPs) at 72% (new TPs helped).")
    print()
    print("  Four improvements and their measured impact (cumulative across versions):")
    print("  1. Interior test points: MCU board 11% → 34% avg (LED TPs unlocked")
    print("     disambiguation of 3 parallel identical strings)")
    print("  2. AC impedance sweep: SHORT coverage 23% → 62% on power board;")
    print("     bridging 0% → 60% on MCU board, 78% on cal board")
    print("  3. k-shortest-paths voting: SHORT +11pp on power board; OPEN/MISSING")
    print("     slightly reduced vs single-path (vote dilution across parallel paths)")
    print("  4. Bayesian priors + precision scoring: marginal on top-1; improves")
    print("     tie-breaking and reduces false positives on high-degree nodes")
    print()
    ...
    print("  1. Adaptive path selection: use single BFS for OPEN-type faults,")
    print("     k-paths only for SHORT/BRIDGE to avoid OPEN vote dilution")
    print("  2. More interior test points (between each LED, at each regulator output)")
    print("  3. Impedance matrix sweep (all N×N TP pairs, not just shortest-path subset)")
    print("  4. AC/transient signature: capacitor and inductor faults invisible at DC")
    print()
