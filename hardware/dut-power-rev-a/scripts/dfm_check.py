"""DFM quality gate — FL-1 DUT Power + Fast-Trip Board Rev A.

Same rules as pcba-rev-a dfm_check.py:
  1. courtyard-to-board-edge >= 3.0 mm
  2. courtyard-to-mounting-hole >= 3.5 mm radial
  3. >= 3 fiducials
  4. courtyard-to-courtyard >= 0.4 mm

Writes elec/layout/dut-power-rev-a/output/dfm_profile_check.json; exits 1 on FAIL.
"""
import json
import os
import re
...
BOARD = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
    ...
    "..", "elec", "layout", "dut-power-rev-a", "dut-power-rev-a.kicad_pcb")
EDGE_MM          = 3.0
HOLE_KEEPOUT_MM  = 3.5
CY_GAP_MM        = 0.4
...
for i, a in enumerate(others):
    for b in others[i + 1:]:
        ...
        if dx < CY_GAP_MM and dy < CY_GAP_MM and max(dx, dy) > -0.01:
            viol["gap"].append(f"{a['ref']} <-> {b['ref']} ({max(dx, dy):.2f} mm)")
