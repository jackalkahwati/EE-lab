/** E8 regression: FL-1 bundle + validation sessions (isolated synthetic artifacts + store). */
import { withEnterpriseFixture } from './enterprise_test_fixture.mjs'

await withEnterpriseFixture('e8', ['fl1'], async (fixture) => {
  fixture.routedRun('fl1-core6-bare-rp2040-combination-v1')
  fixture.artifact('fl1-core6-bare-rp2040-combination-v1', 'fl1-testplan.json', {
    title: 'Synthetic E8 validation plan', steps: [],
  })
  const ent = await fixture.importModule('store')
  const fl1 = await fixture.importModule('fl1')
  ...
  check('2 session planned with synthetic validation plan from run artifacts',
    s.status === 'planned'
  ...
  // Synthetic evidence file used only inside this isolated test.
