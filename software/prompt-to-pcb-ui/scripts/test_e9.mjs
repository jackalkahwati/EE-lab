/** E9 regression: customer-facing report exports (isolated synthetic artifacts + store). */
import { withEnterpriseFixture } from './enterprise_test_fixture.mjs'

await withEnterpriseFixture('e9', ['pilots', 'reports'], async (fixture) => {
  fixture.routedRun('power-entry-header-v1')
  const ent = await fixture.importModule('store')
  const pil = await fixture.importModule('pilots')
  const rep = await fixture.importModule('reports')
