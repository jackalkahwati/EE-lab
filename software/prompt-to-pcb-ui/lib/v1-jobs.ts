...
// runFunctionalWire / runDesignGate live in lib/design-gate.ts (the ONE
// implementation, shared with app/api/pipeline/run/route.ts) — see its CONTRACT.
 ...
 *  parameterized from the actual netlist. Returns {status, failCount, summary}
 *  — status PASS | FAIL | SKIP (nothing evaluable; NOT a pass) — or null when
 *  it did NOT run (`FUNCSIM ERROR`, timeout, python missing, no verdict line),
 *  logged loudly. This is what makes "goes through simulation" mean
...
async function runFunctionalSim(specPath: string): Promise<{ status: 'PASS' | 'FAIL' | 'SKIP'; failCount: number; summary: string } | null> {
  const plannerDir = path.join(process.cwd(), '..', '..', 'hardware', 'planner')
  const notRun = (reason: string): null => { console.error(`[design-gate] not run: functional_sim: ${reason}`); return null }
  return new Promise((resolve) => {
    ...
    } catch (e) { resolve(notRun(`spawn failed: ${String(e).slice(0, 120)}`)); return }
    let out = ''
    let settled = false
    ...
    py.on('error', (e: NodeJS.ErrnoException) => {
      if (settled) return
      settled = true
      resolve(notRun(e.code === 'ENOENT' ? 'python not found' : String(e).slice(0, 120)))
    })
    py.on('close', (code, signal) => {
      if (settled) return
      settled = true
      const err = out.match(/^FUNCSIM ERROR (.*)$/m)
      if (err) { resolve(notRun(err[1].trim().slice(0, 200))); return }
      if (code === null) { resolve(notRun(`killed (${signal ?? 'timeout'})`)); return }
      const sims = out.split('\n').filter((l) => l.startsWith('SIM ')).map((l) => l.replace(/^SIM /, '').trim())
      const summary = sims.map((s) => s.split(/\s+/).slice(0, 2).join(' ')).join(', ').slice(0, 200)
      const fm = out.match(/^FUNCSIM FAIL (\d+)/m)
      if (fm) { resolve({ status: 'FAIL', failCount: Number(fm[1]), summary }); return }
      if (/^FUNCSIM PASS/m.test(out)) { resolve({ status: 'PASS', failCount: 0, summary }); return }
      if (/^FUNCSIM SKIP/m.test(out)) { resolve({ status: 'SKIP', failCount: 0, summary: summary || 'nothing to evaluate' }); return }
      resolve(notRun(`no FUNCSIM line (exit ${code})`))
    })
  })
}
  ...
  try {
    ...
    try {
      ...
      if (hasSpec) {
        ...
        // Both return null when the tool did NOT run (timeout / python missing /
        // `ERROR` verdict / no verdict line) — surfaced in the stage detail with the
        // reason, NEVER treated as a fail or a pass. Only a true GATE FAIL blocks.
        let wireNotRun = ''
        let gateNotRun = ''
        const wired = await runFunctionalWire(specPath, job.prompt, { onNotRun: (r) => { wireNotRun = r } })
        const gate = await runDesignGate(specPath, job.prompt, { onNotRun: (r) => { gateNotRun = r } })
        const wireNote = wired === null
          ? ` · functional-wire not run (${wireNotRun.slice(0, 120)})`
          : wired ? ` · functional-wire +${wired}` : ''
        if (gate && !gate.pass) {
          ...
          // Gate passed (or did not run) → run the FUNCTIONAL sim: the netlist-
          // parameterized ngspice decks (reference, signal chain, driver, PDN) so
          // "goes through simulation" means the real circuit paths, not just
          // thermal. Reported in the stage detail; the critical-path checks carry
          // datasheet-class assumptions so this informs rather than hard-blocks
          // (the connectivity gate is the hard block).
          const fsim = await runFunctionalSim(specPath)
          const cur = job.stages['design plan' as PipeStage] as { status: string; detail: string }
          const pass = gate
            ? ` · design gate PASS${gate.warnCount ? ` (${gate.warnCount} advisory)` : ''}`
            : ` · [design-gate] not run: ${gateNotRun.slice(0, 160)}`
          const fnote = fsim
            ? ` · funcsim ${fsim.status === 'FAIL' ? `FAIL(${fsim.failCount})` : fsim.status}`
            : ' · funcsim not run'
